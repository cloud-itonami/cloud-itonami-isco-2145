(ns chemeng.actor
  "ChemicalEngineersActor — the ISCO-08 2145 chemical engineers actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one chemical engineering operation request
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-2141's
  manufacturing QA actor.
  ...
  The unconditional invariant: the ChemicalEngineersAdvisor can never
  directly commit a record the ChemicalEngineersGovernor refuses —
...
(defn build-graph
  "Build a compiled ChemicalEngineersActor graph. `store` implements
  `chemeng.store/Store`. `advisor` implements
  `chemeng.advisor/Advisor` (defaults to `mock-advisor`).
